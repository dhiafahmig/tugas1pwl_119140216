...
class Cart extends React.Component {
  ...
  chekout = () => {
    Swal.fire({
      title: 'Chekout',
      text: "Apakah anda yakin ingin chekout?",
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
    ...
    return (
      <>
        ...
        {this.props.listBarang.length > 0 ? 
          <table>
              ...
              <tbody>
              {
                  this.props.listBarang.map((barang,key)=>{
                      return(
                          <tr key={key}>    
                          <td><button onClick={()=>{this.hapusBarangAlert(barang.id)}}>X HAPUS</button></td>
              ...
              <tr>
                <td colSpan="5">
                  <div className={style.container_button}>
                    <button className={style.button} onClick={()=>{this.chekout()}} >
                      CHEKOUT
                    </button>
                  </div>
                </td>
          ...
          :
          <h1>Anda Belum Punya Barang di Keranjang</h1>
        }
